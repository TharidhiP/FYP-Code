...
def get_config(config_path):
    # derive the paths to the YOLO configuration 
    configPath = os.path.sep.join([yolo_path, config_path])
    return configPath

def load_model(configpath,weightspath):
    # load the YOLO object detector and classifier trained on OAI dataset
...
def get_predection(image,net,LABELS,COLORS):
    ...
    for output in layerOutputs:
        ...
        for detection in output:
            # extract the class ID and confidence or probability of
    ...
    if len(idxs) > 0:
        # loop over the indexes that is being kept
